<?php 
    ...
    $ac_post_type = "accordion_shortcode";
	
    $AllAccordion = array(  'p' => $WPSM_AC_SH_ID, 'post_type' => $ac_post_type, 'orderby' => 'ASC');
    $loop = new WP_Query( $AllAccordion );
	
	while ( $loop->have_posts() ) : $loop->the_post();
		//get the post id
		$post_id = get_the_ID();
		$Accordion_Shortcode_Settings = unserialize(get_post_meta( $post_id, 'Accordion_Shortcode_Settings', true));
		...
		$accordion_data = unserialize(get_post_meta( get_the_ID(), 'wpsm_accordion_shortcode_data', true));
		$TotalCount =  get_post_meta( get_the_ID(), 'wpsm_accordion_shortcode_count', true );
		if($TotalCount>0) 
		{
		?>
					...
					<div class="wpsm_panel-heading" role="tab" >
					  <h4 class="wpsm_panel-title">
						<a  class="<?php if($i!=1){ echo "collapsed"; } ?>"  data-toggle="collapse" data-parent="<?php if($enable_toggle=="no") { ?>#wpsm_accordion_<?php echo $post_id; ?> <?php } ?>" href="#ac_<?php echo $post_id; ?>_collapse<?php echo $i; ?>"  >
							...
							{ ?>
								<span  class="ac_open_cl_icon fa fa-<?php if($i==$j){ echo "chevron-down"; } else { echo "chevron-right"; } ?>"></span>
	...
	jQuery(document).ready(function() {
			...
			jQuery('.collapse_<?php echo $post_id; ?>').on('shown.bs.collapse', function(){jQuery(this).parent().find(".fa-chevron-right").removeClass("fa-chevron-down").addClass("fa-chevron-down"); jQuery(this).parent().find(".wpsm_panel-heading").addClass("acc-a"); }).on('hidden.bs.collapse', function(){jQuery(this).parent().find(".fa-chevron-down").removeClass("fa-chevron-down").addClass("fa-chevron-right"); jQuery(this).parent().find(".wpsm_panel-heading").removeClass("acc-a");});
