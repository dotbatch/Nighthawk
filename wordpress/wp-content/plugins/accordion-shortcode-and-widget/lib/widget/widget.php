 ...
 * Adds  widget.
 */
class Wpsm_Accordion_Widget extends WP_Widget {
    ...
    function __construct() {
        parent::__construct(
            'wpsm_acsh_widget', // Base ID
            'Accordion Widget', // Name
            array( 'description' => __( 'Display Your Accordion in widget area.', wpshopmart_accordion_shortcode_directory_url ), ) // Args
        );
	}
    ...
    public function widget( $args, $instance ) {
        $Title    	=   apply_filters( 'wpsm_acsh_widget_title', $instance['Title'] );
		echo $args['before_widget'];
		
		 $wpsm_acsh_id	=   apply_filters( 'wpsm_acsh_widget_shortcode', $instance['Shortcode'] ); 

		if(is_numeric($wpsm_acsh_id)) {
			if ( ! empty( $instance['Title'] ) ) {
			echo $args['before_title'] . apply_filters( 'widget_title', $instance['Title'] ). $args['after_title'];
			}
			echo do_shortcode( '[WPSM_AC_SH id='.$wpsm_acsh_id.']' );
		} else {
    ...
    public function form( $instance ) {
        ...
        <p>
			 ...
			 */
			$wpsm_ac_cpt = "Accordion_Shortcode";
			...
			<select id="<?php echo $this->get_field_id( 'Shortcode' ); ?>" name="<?php echo $this->get_field_name( 'Shortcode' ); ?>" style="width: 100%;">
				...
				?>
				<option value="<?php echo $PostId; ?>" <?php if($Shortcode == $PostId) echo 'selected="selected"'; ?>><?php if($PostTitle) echo $PostTitle; else _e("No Title", wpshopmart_accordion_shortcode_directory_url); ?></option>
				<?php endwhile; ?>
				<?php
			}  else  { 
...
// Register Widget
function Wpsm_Accordion_Widget() {
    register_widget( 'Wpsm_Accordion_Widget' );
}
add_action( 'widgets_init', 'Wpsm_Accordion_Widget' );
?>