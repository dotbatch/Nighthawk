...
class pdfth_pdf_thumbnails extends pdfth_core_pdf_thumbnails {
	...
	protected function add_actions() {
		...
		if (is_admin()) {
			...
			add_action(is_multisite() ? 'network_admin_menu' : 'admin_menu', array($this, 'pdfth_admin_menu'));

			if (is_multisite()) {
				add_action('network_admin_edit_'.$this->get_options_menuname(), array($this, 'pdfth_save_network_options'));
			}

			add_filter(is_multisite() ? 'network_admin_plugin_action_links' : 'plugin_action_links', array($this, 'pdfth_plugin_action_links'), 10, 2 );
	...
	public function attachment_fields_to_edit($form_fields, $post) {
		if ($post->post_mime_type == 'application/pdf') {
			...
			if ($thumbnail) {
				$html = sprintf("<span id='%s'><a href='#' onclick='javascript: %s; return false;'>Regenerate thumbnail</a></span>",
					$spanid,
					esc_attr($jscode));
			}
			else {
