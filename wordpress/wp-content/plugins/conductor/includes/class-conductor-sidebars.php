...
if( ! class_exists( 'Conductor_Sidebars' ) ) {
	final class Conductor_Sidebars {
		 ...
		 */
		// TODO: Sidebar descriptions are created using the $content_layout['field_label'] instead of the $sidebar_name_prefix (should we switch?)
		// TODO: Change <section> wrapper elements to <div>
		public static function register_conductor_sidebars( $content_layout, $content_layouts, $force_registration = false ) {
			 ...
			 */
			$sidebar_args = array(
				'name'          => ( isset( $sidebar_name_suffixes['content'] ) ) ? $sidebar_name_prefix . ' - ' . $sidebar_name_suffixes['content'] : $sidebar_name_prefix . ' - Content', // TODO: i18n?
				'id'            => Conductor::get_conductor_content_layout_sidebar_id( 'content', $content_layout ),
				'description'   => sprintf( __( 'This widget area is the content widget area for %1$s.', 'conductor' ), $content_layout['field_label'] ),
				'before_widget' => '<section id="' . $content_layout['field_type'] . '-' . $content_layout['field_id'] . '-widget-%1$s" class="widget ' . $content_layout['field_type'] . '-' . $content_layout['field_id'] . ' ' . $content_layout['field_type'] . '-' . $content_layout['field_id'] . '-widget content-sidebar content-sidebar-widget %2$s">',
			...
			if ( conductor_content_layout_has_sidebar( 'primary', $content_layout ) || $force_registration ) {
				$sidebar_args = array(
					'name'          => ( isset( $sidebar_name_suffixes['primary'] ) ) ? $sidebar_name_prefix . ' - ' . $sidebar_name_suffixes['primary'] : $sidebar_name_prefix . ' - Primary', // TODO: i18n?
					'id'            => Conductor::get_conductor_content_layout_sidebar_id( 'primary', $content_layout ),
					'description'   => sprintf( __( 'This widget area is the primary widget area for %1$s.', 'conductor' ), $content_layout['field_label'] ),
					'before_widget' => '<section id="' . $content_layout['field_type'] . '-' . $content_layout['field_id'] . '-widget-%1$s" class="widget ' . $content_layout['field_type'] . '-' . $content_layout['field_id'] . ' ' . $content_layout['field_type'] . '-' . $content_layout['field_id'] . '-widget primary-sidebar primary-sidebar-widget %2$s">',
					'after_widget'  => '</section>',
			...
			if ( conductor_content_layout_has_sidebar( 'secondary', $content_layout ) || $force_registration ) {
				$sidebar_args = array(
					'name'          => ( isset( $sidebar_name_suffixes['secondary'] ) ) ? $sidebar_name_prefix . ' - ' . $sidebar_name_suffixes['secondary'] : $sidebar_name_prefix . ' - Secondary', // TODO: i18n?
					'id'            => Conductor::get_conductor_content_layout_sidebar_id( 'secondary', $content_layout ),
					'description'   => sprintf( __( 'This widget area is the secondary widget area for %1$s.', 'conductor' ), $content_layout['field_label'] ),
					'before_widget' => '<section id="' . $content_layout['field_type'] . '-' . $content_layout['field_id'] . '-widget-%1$s" class="widget ' . $content_layout['field_type'] . '-' . $content_layout['field_id'] . ' ' . $content_layout['field_type'] . '-' . $content_layout['field_id'] . '-widget secondary-sidebar secondary-sidebar-widget %2$s">',
					'after_widget'  => '</section>',
		...
		public function array_filter_find_conductor_widgets( $var ) {
			return _get_widget_id_base( $var ) === 'conductor-widget';
		}
		 ...
		 * parameters and only if the sidebars are currently empty.
		 *
		 *  * $args = array(
