...
if ( ! class_exists( 'Conductor_Customizer' ) ) {
	final class Conductor_Customizer {
		...
		private function includes() {
			include_once( 'customizer/class-conductor-customizer-content-layouts-setting.php' ); // Conductor Theme Customizer Content Layouts Setting
			include_once( 'customizer/class-conductor-customizer-content-layouts-control.php' ); // Conductor Theme Customizer Content Layouts Control
		}
		...
		public function customize_preview_init() {
			...
			add_action( 'wp_footer', array( $this, 'wp_footer' ), 20 ); // Output Conductor Content Layout settings
		...
		/**
		 * This function outputs the current page's Conductor data (if any).
		 */
		// TODO: Use wp_localize_script() instead
		public function wp_footer() {
			// Content layout (if Conductor)
			if ( Conductor::is_conductor( false ) ) {
				$conductor_options = array(
				...
				$conductor_options['content_layout']['sidebar_prefix'] = Conductor::get_conductor_content_layout_sidebar_id_prefix( '', Conductor::get_conductor_content_layout() );

				// Conductor sidebar suffix
				$conductor_options['content_layout']['sidebar_suffix'] = Conductor::get_conductor_content_layout_sidebar_id_suffix( '', Conductor::get_conductor_content_layout() );
			}
			// Content Layout (non-Conductor, need to populate layout properties)
			else {
				$permalink_structure = get_option( 'permalink_structure' ); // Permalink Structure
				$conductor_options = array(
				...
				if ( is_front_page() && ! is_home() ) {
					$conductor_options['content_layout']['field_label'] = $conductor_options['content_layout']['sidebar_name_prefix'] = __( 'Front Page', 'conductor' );
					$conductor_options['content_layout']['field_type'] = 'built-in';
					$conductor_options['content_layout']['field_id'] = 'front_page';

					// Permalink
					$conductor_options['content_layout']['permalink'] = trailingslashit( home_url() );
				}
				// Home/Blog (Built-In)
				else if ( is_home() ) {
					$conductor_options['content_layout']['field_label'] = $conductor_options['content_layout']['sidebar_name_prefix'] = __( 'Blog', 'conductor' );
					$conductor_options['content_layout']['field_type'] = 'built-in';
					$conductor_options['content_layout']['field_id'] = 'home';

					// Permalink
					if ( $page_for_posts = get_option( 'page_for_posts' ) )
						$conductor_options['content_layout']['permalink'] = ( $permalink_structure ) ? trailingslashit( get_permalink( $page_for_posts ) ) : get_permalink( $page_for_posts );
					else
						$conductor_options['content_layout']['permalink'] = trailingslashit( home_url() );
				}
				// Category
					...
					// TODO: i18n, l10n
					$conductor_options['content_layout']['field_label'] = 'Category - ' . $category->name;
					$conductor_options['content_layout']['sidebar_name_prefix'] = $category->name . ' - Category';
					$conductor_options['content_layout']['field_type'] = 'category';
					$conductor_options['content_layout']['field_id'] = $category->term_id;

					// Permalink
					$conductor_options['content_layout']['permalink'] = ( $permalink_structure ) ? trailingslashit( get_category_link( $category->term_id ) ) : get_category_link( $category->term_id );
				}
				// Post Type Archive
					...
					// TODO: i18n, l10n
					$conductor_options['content_layout']['field_label'] = 'Post Type - ' . $post_type->labels->name;
					$conductor_options['content_layout']['sidebar_name_prefix'] = $post_type->labels->name . ' - Post Type';
					$conductor_options['content_layout']['field_type'] = 'post-type';
					$conductor_options['content_layout']['field_id'] = $post_type->name;

					// Permalink
					$conductor_options['content_layout']['permalink'] = ( $permalink_structure ) ? trailingslashit( get_post_type_archive_link( $post_type->name ) ) : get_post_type_archive_link( $post_type->name );
				}
				// Single Post
				else if ( is_single() ) {
					...
					// TODO: i18n, l10n
					$conductor_options['content_layout']['field_label'] = 'Post - ' . $post->post_title;
					$conductor_options['content_layout']['sidebar_name_prefix'] = $post->post_title . ' - Post';
					$conductor_options['content_layout']['field_type'] = 'post';
					$conductor_options['content_layout']['field_id'] = $post->ID;

					// Permalink
					$conductor_options['content_layout']['permalink'] = ( $permalink_structure ) ? trailingslashit( get_permalink() ) : get_permalink();
				}
				// Single Page
					...
					// TODO: i18n, l10n
					$conductor_options['content_layout']['field_label'] = 'Page - ' . $page->post_title;
					$conductor_options['content_layout']['sidebar_name_prefix'] = $page->post_title . ' - Page';
					$conductor_options['content_layout']['field_type'] = 'page';
					$conductor_options['content_layout']['field_id'] = $page->ID;

					// Permalink
					$conductor_options['content_layout']['permalink'] = ( $permalink_structure ) ? trailingslashit( get_permalink() ) : get_permalink();
				}

				// No value (i.e. "Disabled" layout)
				$conductor_options['content_layout']['value'] = false;

				// New content layout flag
				$conductor_options['content_layout']['new_content_layout'] = true;
			}

			// Allow filtering of options
			$conductor_options = apply_filters( 'conductor_customizer_preview_options', $conductor_options, Conductor::is_conductor( false ) );
		?>
			<script type="text/javascript">
				// <![CDATA[
					var conductorOptions = <?php echo json_encode( $conductor_options ); ?>;
				// ]]>
			</script>
		<?php
		}
		...
		public function conductor_query_paginate_links_args( $paginate_links_args, $query, $echo, $query ) {
			global $wp;
		...
		public function register_conductor_sidebars() {
			...
			if ( isset( $conductor_options['content_layouts'] ) && ! empty( $conductor_options['content_layouts'] ) )
				foreach( $conductor_options['content_layouts'] as $content_layout )
					...
					Conductor_Sidebars::register_conductor_sidebars( $content_layout, $conductor_options['content_layouts'], true );
		}
