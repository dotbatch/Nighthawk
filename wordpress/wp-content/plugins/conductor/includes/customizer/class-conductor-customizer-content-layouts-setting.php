<?php
/**
 * Conductor Customizer Content Layouts Setting (Theme Customizer functionality)
 *
...
if( ! class_exists( 'Conductor_Customizer_Content_Layouts_Setting' ) ) {
	final class Conductor_Customizer_Content_Layouts_Setting extends WP_Customize_Setting {
		...
		/**
		 * This function updates the Conductor option based on the value.
		 */
		protected function _update_option( $value ) {
			// Grab Conductor Options
			$options = get_option( $this->id_data['base'] );

			$conductor_content_layouts = $this->multidimensional_get( $options, $this->id_data['keys'] );
			$conductor_post_value = $this->conductor_post_value( $value );

			// Check to see if the options differ from posted value and vise-versa
			if ( $this->array_diff_assoc_recursive( ( array ) $conductor_content_layouts, $conductor_post_value ) || $this->array_diff_assoc_recursive( ( array ) $conductor_post_value, $conductor_content_layouts ) ) {
				$options = $this->multidimensional_replace( $options, $this->id_data['keys'], $value );

				// If we actually have a difference in options after replacing
				if ( $options )
					// sanitize_option() function in Conductor_Admin_Options will be called here
