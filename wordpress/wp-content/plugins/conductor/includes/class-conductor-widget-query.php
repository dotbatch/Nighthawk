...
if( ! class_exists( 'Conductor_Widget_Query' ) ) {
	class Conductor_Widget_Query {
		...
		/**
		 * This function is used to set a single content piece by ID.
		 */
		...
		/**
		 * This function is used to get a single content piece by ID.
		 */
		...
		?>
			<<?php echo $this->get_content_wrapper_html_element( $post, $instance, $widget, $query ); ?> class="<?php echo apply_filters( 'conductor_widget_content_wrapper_css_classes', 'content post-content conductor-cf', $post, $instance, $widget, $query ); ?><?php echo ( ! $instance['hide_post_thumbnails'] && has_post_thumbnail( $post->ID ) ) ? ' has-post-thumbnail content-has-post-thumbnail' : false; ?>">
		<?php
		}
