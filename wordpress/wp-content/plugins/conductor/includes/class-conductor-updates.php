...
if( ! class_exists( 'Conductor_Updates' ) ) {
	final class Conductor_Updates {
		...
		/**
		 * @var EDD_SL_Plugin_Updater, Instance of the EDD Software Licensing Plugin Updater class
		 */
		protected $updater;
		...
		private function includes() {
			if ( ! class_exists( 'EDD_SL_Plugin_Updater' ) )
				include_once( 'class-edd-sl-plugin-updater.php' ); // EDD Software Licensing Plugin Updater
		}
		...
		public function admin_init() {
			...
			$api_data = ( ! empty( $this->api_data ) ) ? array_merge( $api_data, $this->api_data ) : $api_data;

			// Create and instance of the plugin updater
			$this->updater = new EDD_SL_Plugin_Updater( $this->get_url(), $this->get_plugin_file(), $api_data );
		}
