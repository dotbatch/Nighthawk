...
// TODO: Replace all references of id_base (conductor-widget) with value from localized data
...
( function ( wp, $ ) {
	...
	if ( conductor.hasOwnProperty( 'widgets' ) && ! conductor.widgets.hasOwnProperty( 'conductor' ) ) {
		$.extend( conductor.widgets, { conductor: {} } );
	}
	else {
		conductor.widgets = conductor.widgets || { conductor: {} };
	}
	...
	$( function() {
		...
		$document.on( 'change', '.conductor-content-type', function( e ) {
			...
			if ( value ) {
				...
				// Show correct fields
				// TODO: use filter() or other functionality instead
		...
		// Populate CSS ID for each widget
		// TODO
		/*$( 'div.widgets-sortables' ).children( '.widget' ).each( function() {
			var $this = $( this ), widget_css_id = $this.parent().attr( 'id' ).replace( '-sidebar', '' ) + this.id.match( /-widget-\d+/ );

			// If we have a widget CSS ID
			if ( widget_css_id ) {
				$this.find( 'input.conductor-css-id' ).val( widget_css_id );
			}
		} );*/

		// Backwards compatibility for WordPress versions less than (<) 3.9
		// TODO: Do we need backwards compatibility? Widget support in Customizer was added in 3.9
		if ( parseFloat( conductor.widgets.conductor.wp_version ) < 3.9 ) {
			$document.ajaxSuccess( function( event, XMLHttpRequest, ajaxOptions ) {
				var ajax_data = $.unserialize( ajaxOptions.data );

				// If we have an action and it's the "save-widget" action, lets trigger the accordionInit() function
				if ( ajax_data.action.length && ajax_data.action === 'save-widget' ) {
					$widgets = $( '.widget', '#widgets-right' );
					conductor.widgets.conductor.accordionInit( $widgets );
				}
			} );
		...
		output: Backbone.View.extend( {
			...
			saveElementLabel: function( event ) {
				...
				if ( event.type === 'keypress' && event.which === 13 ) {
					var $output_element = $el.closest( '.conductor-widget-output-element' ),
			...
			sortableStop: function( event, ui ) {
				...
				$conductor_output_data.val( JSON.stringify( data ) ).trigger( 'change' );
			},
