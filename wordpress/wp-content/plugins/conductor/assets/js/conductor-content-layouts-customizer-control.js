/**
 * Conductor Content Layouts Theme Customizer Control
 */
	...
	api.conductorContentLayoutsControl = api.Control.extend( {
		...
		ready: function() {
			...
			$( function() {
				...
				// TODO: Remove (refresh)?
				self.previewerURL = api.Conductor.Previewer.previewUrl(); // Store a reference to the currentPreviewer URL
			} );
			...
			// Listen for the "conductor-sidebars" event from the Previewer and trigger the same event name on this controller's main Backbone view to setup content layout data
		...
		setPreviewerContentLayout: function( content_layout, self ) {
			...
			// Reset the previewer URL reference
			// TODO: Remove (refresh)?
			self.previewerURL = api.Conductor.Previewer.previewUrl();
		},
		...
		views: {},
		saved: false, // TODO: Remove (refresh)?
		previewerURL: false,
			...
			maybeAddContentLayout: function( event ) {
				...
				if ( ! this.$( '.conductor-content-types' ).val() && this.self.conductor_content_layout.hasOwnProperty( 'new_content_layout' ) ) {
					...
					if ( ! this.conductor_content_layout_controls_view.$( '.conductor-content-layout-' + this.self.conductor_content_layout.field_type + '-' + this.self.conductor_content_layout.field_id ).length ) {
						...
						// TODO: i18n; l10n
						this.conductor_content_layout_controls_view.showMessage( 'New content layout was created successfully. Loading...', 'content-layout-created' );
				...
				// New content layout and content type is selected
					...
					if ( ! content_type.id.length || ! content_type.name.length || ! content_type.type.length ) {
						// TODO: i18n; l10n
						this.conductor_content_layout_controls_view.showMessage( 'Please select a content type.', 'no-content-type', 'error' );
					...
					// Content layout exists, need to re-direct Previewer
						...
						// Store the current previewer URL
						// TODO: Remove (refresh)?
						self.self.previewerURL = api.Conductor.Previewer.previewUrl();
		...
		set: function( setting ) {
			// TODO: Remove (refresh)?
			this.saved = ( api.state( 'saved' )() ) ? true : false;
	...
	api.Widgets.ConductorWidgetControl = api.Widgets.WidgetControl.extend( {
		...
		updateWidget: _.debounce( function( args ) {
			var self = this, completeCallback, $widgetContent,
				...
				l10n = conductorCustomizeWidgetsSettings.l10n;
