	...
	conductor['content-layouts'].views['content-layout-controls'] = Backbone.View.extend( {
		...
		maybeAddContentLayout: function( event ) {
			...
			if ( ! content_type.id.length || ! content_type.name.length || ! content_type.type.length || ( conductor.hasOwnProperty( 'is_customizer' ) && conductor.is_customizer === '1' ) ) {
				// TODO: i18n; l10n
				this.showMessage( 'Please select a content type.', 'no-content-type', 'error' );
			...
			if ( ! this.controller_view.$( '.conductor-content-layout-' + content_type.type + '-' + content_type.id ).length ) {
				...
				if ( this.getAddContentLayoutFlag() ) {
					...
					// TODO: i18n; l10n
					this.showMessage( 'New content layout was created successfully.', 'content-layout-created' );
				}
				// Reset the flag
				else {
				...
				// TODO: i18n; l10n
				this.showMessage( 'A content layout for that content type already exists. Please try again.', 'content-layout-exists', 'error' );
			}
		},
		addContentLayout: function( model, content_type ) {
			...
			if ( ( ! model.hasOwnProperty( 'field_label' ) || ! model.field_label ) && content_type.hasOwnProperty( 'name' ) ) {
				switch( model.field_type ) {
					...
					// Category
					case 'category':
						model.field_label = 'Category - ' + content_type.name;
						break;

					// Post Type
					case 'post-type':
						model.field_label = 'Post Type - ' + content_type.name;
						break;

					// Post
					case 'post':
						model.field_label = 'Post - ' + content_type.name;
						break;

					// Page
					case 'page':
						model.field_label = 'Page - ' + content_type.name;
						break;
				}
			}
