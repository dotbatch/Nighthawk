...
if ( ! class_exists( 'Conductor' ) ) {
	final class Conductor {
		 ...
		 */
		function conductor_note_widgets_init() {
			// Conductor Widget
			include_once( 'includes/widgets/class-conductor-widget.php' );
		 ...
		 * templates into to override Conductor default templates (i.e. your-theme/conductor/).
		 *
		 ...
		 * It will likely only function after the plugins_loaded action, once the PHP class is included.
		 *
		 * @return Boolean
		 */
		 ...
		 * @return Boolean
		 */
		// TODO: Should $ignore_default be renamed?
		public static function is_conductor( $ignore_default = true ) {
			// Return Conductor status based off of $ignore_default (if we have a content layout)
			if ( ! $ignore_default && Conductor::get_conductor_content_layout() )
				return true;
			...
			if ( ! empty( $conductor_options['content_layouts'] ) && is_array( $conductor_options['content_layouts'] ) ) {
				// Static Front Page (not "Your latest posts")
				...
				// Single Post
				if ( is_single() )
					$conductor_content_layout = array_filter( $conductor_options['content_layouts'], array( get_class(), 'array_filter_content_layout_field_id_is_post' ) );

				// Single Page
				if ( is_page() && ! is_front_page() )
					$conductor_content_layout = array_filter( $conductor_options['content_layouts'], array( get_class(), 'array_filter_content_layout_field_id_is_page' ) );
			...
			if ( ! empty( $conductor_content_layout ) && is_array( $conductor_content_layout ) ) {
				...
				if ( ( isset( $conductor_content_layout['value'] ) && ! empty( $conductor_content_layout['value'] ) ) || ! $ignore_default ) {
			...
			self::$is_conductor = apply_filters( 'conductor_is_conductor', self::$is_conductor, $conductor_content_layout, $conductor_options );
		...
		public static function get_conductor_content_layout_data( $content_layout = false ) {
			...
			$conductor_content_layouts = Conductor_Options::get_content_layouts(); // Grab all registered Conductor content layouts
			$conductor_content_layout_data = ( isset( $conductor_content_layouts[$content_layout_value] ) ) ? $conductor_content_layouts[$content_layout_value] : array();
