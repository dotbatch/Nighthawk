	...
	public function wpe_cdn_srcset( $sources ) {
		...
		if ( $cdn_domain ) {
			foreach ( $sources as $source ) {
				...
				$sources[ $source['value'] ]['url'] = str_replace( $blog_url, $full_cdn_domain, $sources[ $source['value'] ]['url'] );
			}
		}
...
//		if (isset($msg['body'])) return $msg['body'];
    ...
    public function get_site_info() {
        ...
        if ( ! $cached_site_info ) {
            ...
            $r->sftp_host = ($r->is_pod ? $r->name : "sftp".$r->cluster) . ".wpengine.com";
...
// Should we cache database queries for logged-in users?
    ...
    public static function clear_maxcdn_cache() {
        ...
        $url = sprintf( "https://api2.wpengine.com/install/%s/cdn/purge", PWP_NAME );
        $args = array(
            'timeout' => 3,
            'blocking' => true,
            'headers' => array (
