...
if (isset($motor_options)) {
	add_filter( 'loop_shop_per_page', create_function( '$cols', 'return '.$motor_options["motor_catalog_pagi"].';' ), 20 );
}
...
add_filter( 'woocommerce_add_to_cart_fragments', 'motor_header_add_to_cart_fragment_cart' );
function motor_header_add_to_cart_fragment_cart( $fragments ) {
	ob_start();
	?>
...
add_action('woocommerce_cart_updated', 'motor_wac_update');
function motor_wac_update() {
    if ( !empty($_POST['is_cart_ajax'])) {
        $resp = array();
        $resp['update_label'] = esc_html__( 'Update Cart', 'motor' );
        $resp['checkout_label'] = esc_html__( 'Proceed to Checkout', 'motor' );
        $resp['price'] = 0;
