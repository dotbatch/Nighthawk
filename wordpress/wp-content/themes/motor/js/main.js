...
jQuery(document).ready(function ($) {
	...
	if ($('#section-filter-toggle-btn').length > 0 && $('#section-filter .woof_redraw_zone').length > 0) {
		$('.section-filter-toggle').on('click', '#section-filter-toggle-btn', function () {
			if ($(this).parent().hasClass('filter_hidden')) {
				$(this).text('Hide Filter').parent().removeClass('filter_hidden');
				//$('#section-filter .woof_redraw_zone').slideDown();
				document.cookie = "filter_toggle=filter_opened; expires=Thu, 31 Dec 2020 23:59:59 GMT; path=/;";
			} else {
				$(this).text('Show Filter').parent().addClass('filter_hidden');
	...
	if ($('.blog-sb-widget select').length > 0) {
		$('.blog-sb-widget select').styler({
			selectPlaceholder: 'Select'
		});
	}
		...
		$('body').on('click', '.qnt-wrap a', function () {
			...
			if (qnt > 0) {
				if ($(this).parents('.sectls').find('.prod-li-total').length > 0) {
					$(this).parents('.sectls').find('.prod-li-total').text('$' + qnt*$(this).parent().find('input').attr('data-qnt-price'));
				}
				if ($(this).parents('.prod-cont').find('.prod-total-wrap .prod-price').length > 0) {
					$(this).parents('.prod-cont').find('.prod-total-wrap .prod-price').text('$' + qnt*$(this).parent().find('input').attr('data-qnt-price'));
